...
- **対象サーバー**（4 つ。いずれも再現性のためバージョンを固定する）:
  - `serena`: セマンティックなコード検索/編集 (LSP)。PyPI `serena-agent==1.5.3` を `uvx` で実行。設定は `.serena/project.yml`。
  - `context7`: ライブラリ公式ドキュメント取得 (`@upstash/context7-mcp@3.1.0`)。API キーは任意（無しでも匿名動作）。
  - `playwright`: ブラウザ自動操作 (`@playwright/mcp@0.0.75`)。`npx playwright install` でブラウザ導入が必要。
  - `chrome-devtools`: Chrome DevTools 連携 (`chrome-devtools-mcp@1.1.1`)。Chrome/Chromium が必要。
- **設定ファイル**:
...
- **バージョン固定 / 更新**: `@latest` や git の HEAD 追従は使わず、各パッケージを正確なバージョンに
  ピン留めする（`.npmrc` の `save-exact` / `min-release-age=7` と同じ再現性・サプライチェーンの方針）。
  更新する際は 3 つの設定ファイル（`.mcp.json` / `.gemini/settings.json` / `.codex/config.toml`）の
  バージョンを揃え、動作確認してからコミットする。
